...
clear;clear;clear
  ...
  # // Banner
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo -e "  Welcome To Rifqi vpn Tunneling ${YELLOW}(${NC}${green} Stable Edition ${NC}${YELLOW})${NC}"
echo -e " This Will Quick Setup VPN Server On Your Server"
echo -e "  Auther : ${green}Rifqi vpn® ${NC}${YELLOW}(${NC} ${green} Rifqi vpn Tunneling ${NC}${YELLOW})${NC}"
echo -e " © Recode By My Self Rifqi Tunneling${YELLOW}(${NC} 2023 ${YELLOW})${NC}"
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo ""
sleep 2
###### IZIN SC 
...
# USERNAME
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/rifstore/izinsc/main/ip | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/rifstore/izinsc/main/ip | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
# DETAIL ORDER
...
Exp1=$(curl https://raw.githubusercontent.com/rifstore/izinsc/main/ip | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
clear
# REPO    
    REPO="https://raw.githubusercontent.com/rifstore/RIFQIV5/main/"
...
echo -e "     \e[1;32m2)\e[0m Gunakan Auto pointing Domain️ "
echo -e "   ------------------------------------"
...
#install cf
wget -q https://raw.githubusercontent.com/rifstore/RIFQIVIP01/main/pointing.sh && chmod +x pointing.sh && ./pointing.sh
rm -f /root/pointing.sh
clear
else
echo "Pilih antara angka 1 dan 2 !!!"
sleep 2
pasang_domain
fi
}
...
wget raw.githubusercontent.com/rifstore/RIFQIV5/main/limit/limit.sh && chmod +x limit.sh && ./limit.sh
...
function menu(){
    ...
    wget -q https://raw.githubusercontent.com/rifstore/RIFQIVIP01/main/menu.zip
    #unzip menu.zip
    7z x -pScV1Rif menu.zip
    chmod +x menu/*
